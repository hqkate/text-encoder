...
from t5_encoder.t5_config import T5_SUPPORT_LIST, T5Config
from t5_encoder.t5_tokenizer import T5Tokenizer as T5T
...
class T5Tokenizer(T5T):
    ...
    def tokenize_truncate_pad(
        self, text: str, max_length=None, padding=True, truncation=False
    ):
        ...
        start = (
            ...
            else self.rng(0, len(tokens) - output_length + 1)
        )
    ...
    def __call__(
        ...
        for text_input in batch_text_input:
            token, mask = self.tokenize_truncate_pad(
                text_input, max_length, padding, truncation
            )
            tokens.append(token)
            masks.append(mask)
...
def get_t5_tokenizer():
    tokenizer = AutoTokenizer.from_pretrained("models--DeepFloyd--t5-v1_1-xxl")
    return tokenizer


def get_t5_encoder(trainable=False, pretrained=None):
    with open(os.path.join("models--DeepFloyd--t5-v1_1-xxl/config.json"), "r") as file:
...
class T5Attention(nn.Cell):
    ...
    def _relative_position_bucket(
        ...
        if bidirectional:
            ...
            relative_position = 0 - ops.minimum(
                relative_position, ops.zeros(relative_position.shape)
            ).astype(ms.int64)
        ...
        relative_position_if_large = max_exact + (
            ops.log(relative_position.astype(ms.float32) / max_exact) / math.log(max_distance / max_exact) * (num_buckets - max_exact)
        ).astype(ms.int64)
        relative_position_if_large = ops.minimum(
            relative_position_if_large,
            ops.fill(
                relative_position_if_large.dtype,
                relative_position_if_large.shape,
                num_buckets - 1,
            ),
        )
    ...
    def compute_bias(self, query_length, key_length):
        ...
        values = values.transpose([2, 0, 1]).expand_dims(
            0
